...
const TABS: Tab[] = [
  { key: "home", href: "/sso/home", Icon: Home },
  { key: "reports", href: "/sso/reports", Icon: NotebookPen },
  { key: "phonecalls", href: "/sso/phonecalls", Icon: PhoneCall },
  { key: "messages", href: "/sso/messagingChannel", Icon: MessageCircleMore },
  { key: "sop", href: "/sso/sop", Icon: ListChecks },
];
...
export default function BottomBar() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(createRealtimeTopic(`sso-bottom-bar-unread:${currentUserId}`))
      .on(
        "postgres_changes",
        {
          event: "*",
    ...
    <View style={styles.wrap}>
      <View style={styles.bar}>
        {TABS.map(({ key, href, Icon }) => {
          const active = pathname === href || (key === "messages" && pathname === "/sso/message");

          return (
            <Pressable key={key} onPress={() => router.replace(href)} style={styles.btn} hitSlop={10}>
