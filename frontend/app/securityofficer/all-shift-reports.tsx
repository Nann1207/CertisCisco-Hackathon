...
export default function AllShiftReportsScreen() {
  ...
  const pendingReports = useMemo(
    () =>
      shiftRows.filter(
        (row) =>
          row.clockout_time && !(row.shift_description ?? "").trim()
      ),
    [shiftRows]
  );
    ...
    <SafeAreaView style={styles.root}>
      <View style={styles.header}>
        <Pressable
          ...
          onPress={() => (router.canGoBack() ? router.back() : router.replace("/securityofficer/reports"))}
        >
            ...
            pendingReports.map((shift) => (
              <View key={`pending-${shift.id}`} style={styles.card}>
                ...
                <Pressable
                  ...
                  onPress={() => router.push(`/securityofficer/shift-report?shiftId=${shift.id}`)}
                >
