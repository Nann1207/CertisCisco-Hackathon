"""
Report how many shifts are assigned per `officer_id` and map to employee names when possible.

Run with:
  python backend/report_shifts_by_officer.py
"""
...
def main():
    ...
    shifts_resp = sb.table("shifts").select("shift_id,officer_id,shift_date,location").limit(5000).execute()
    shifts = shifts_resp.data or []
    counts = Counter()
    for s in shifts:
        counts[str(s.get("officer_id") or "<NULL>")] += 1
